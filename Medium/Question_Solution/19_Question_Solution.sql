...
Avg_duration as(
select ci.caller_id, left(phone_number, 3)as phone_code from caller_id ci join Person p on ci.caller_id = p.id
group by ci.caller_id, left(phone_number, 3) having AVG(duration) > (select AVG(duration) from caller_id)
)
select distinct ct.name as country from Avg_duration avg_du join Country ct on ct.country_code = avg_du.phone_code;